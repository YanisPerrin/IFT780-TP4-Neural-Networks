...
class CNNTrainTestManager(object):
    ...
    def train(self, num_epochs):
        ...
        resume_epoch = 0  # Will be used to resume training from a checkpoint.
        # Load checkpoint if needed
        if self.load_checkpoint is not None:
            resume_epoch = self._load_checkpoint() + 1
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            if self.save_checkpoint:
                print(f'best_val_acc: {best_val_acc} | val_acc: {self.metric_values["val_acc"][-1]}')
                if best_val_acc < self.metric_values['val_acc'][-1]:
                    print("Saving model...")
                    best_val_acc = self.metric_values['val_acc'][-1]
    ...
    def _load_checkpoint(self) -> int:
        ...
        Returns:
            epoch: the epoch where the model was saved.
        """
        ...
        print(f'Model loaded from {self.load_checkpoint} at epoch {epoch}.')
        return epoch

    def _save_checkpoint(self, epoch: int):
        """
        Save the model's weights after training in 'checkpoints' folder.

        See: https://pytorch.org/tutorials/recipes/recipes/saving_and_loading_a_general_checkpoint.html
        """
        file_name = f'checkpoints/{self.model.__class__.__name__}.pt'
